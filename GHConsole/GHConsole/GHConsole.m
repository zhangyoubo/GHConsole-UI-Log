...
@property (nonatomic, strong) UIButton *moveBottomButton;
...
- (void)doubleTapTextView:(UITapGestureRecognizer *)tapGesture{
    ...
    if (_isFullScreen == NO) {//变成全屏
        ...
        _moveBottomButton.hidden = NO;
      //  [self.textField scrollRangeToVisible:NSMakeRange(self.textField.text.length, 1)]; // zyb
...
- (GHConsoleTextField *)textField{
    if (!_textField) {
        ...
        /**zyb**/
        _stopButton = [[UIButton alloc] initWithFrame:CGRectMake(k_WIDTH - 70, k_HEIGHT/2, 60, 30)];
        [_stopButton setTitle:@"已开始" forState:UIControlStateNormal];
        [_stopButton setTitle:@"已暂停" forState:UIControlStateSelected];
        [_stopButton setTitleColor:[UIColor orangeColor]  forState:UIControlStateNormal];
        [_stopButton setTitleColor:[UIColor redColor]  forState:UIControlStateSelected];
        [_stopButton addTarget:self action:@selector(stopLog:) forControlEvents:UIControlEventTouchUpInside];
        
        _moveBottomButton = [[UIButton alloc] initWithFrame:CGRectMake(k_WIDTH - 70,k_HEIGHT/2 + 50, 60, 30)];
        [_moveBottomButton setTitle:@"到底部" forState:UIControlStateNormal];
        [_moveBottomButton setTitleColor:[UIColor orangeColor]  forState:UIControlStateNormal];
        [_moveBottomButton setTitleColor:[UIColor redColor] forState:UIControlStateSelected];
        [_moveBottomButton addTarget:self action:@selector(moveBottom:) forControlEvents:UIControlEventTouchUpInside];
        /**zyb**/
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            if (_isShowConsole) {
                ...
                [[UIApplication sharedApplication].keyWindow addSubview:_stopButton];
                [[UIApplication sharedApplication].keyWindow addSubview:_moveBottomButton];
                _stopButton.hidden = YES;
                _moveBottomButton.hidden = YES;
            }
        });
...
-(void)stopLog:(UIButton *)sender{
    sender.selected = ! sender.selected;
}

-(void)moveBottom:(UIButton *)sender{
    sender.selected = NO;
    [self.textField scrollRectToVisible:CGRectMake(0, _textField.contentSize.height-15, _textField.contentSize.width, 10) animated:YES];
}
